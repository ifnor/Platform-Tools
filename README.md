...
A `429 Too Many Requests` response during startup is a Cloudflare management API limit, not proof that the cloudflared update damaged credentials. Named-tunnel lookups use a server-side name filter; account verification also uses a filtered query instead of enumerating every tunnel. Within the app, cloudflared management commands are serialized and a 429 starts a five-minute cooldown (the CLI does not expose the server's Retry-After header). During that cooldown, further management commands fail locally with a retry-time message instead of sending additional requests. Running tunnels are unaffected. Wait before retrying; shared-account or IP activity outside this app may prolong the limit. Credentials and service definitions are retained.
